    ...
    pico.run(appCfg,function(){
        ...
        return function(){
            ...
            load=function(keys,deps,cb){
                ...
                require(v, function(err, mod){
                    if (err) return cb(err)
                    mod.setup(ctx, function(err){
                        if (err) return cb(err)
                        deps[k]=mod
                        load(keys,deps,cb)
                    })
            ...
            setup=function(list,cb){
                ...
                list.shift().setup(ctx,function(err){
                    if (err) return cb(err)
                    setup(list,cb)
                })
            }

            load(Object.keys(apis.deps), apis.deps, function(err){
                if (err) return console.error(err)
                setup(Object.getPrototypeOf(models), function(err){
                    if (err) return console.error(err)
