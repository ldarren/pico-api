...
module.exports = {
    ...
    ajax: function(method, href, params, headers, cb, userData){
        ...
        var req = protocol.request(options, (res)=>{
            ...
            if (-1 < [301,302,303,305,306,307,308].indexOf(st)) return module.exports.ajax(method,res.headers.location,params,headers,cb,userData)
            res.setEncoding('utf8')
            var 
            data = '',
            status=(200 === st || !st) ? null : {error:xhr.statusMessage,code:st}
            res.on('data', (chunk)=>{
                data += chunk
                cb(status, 3, data, userData)
            })
            res.on('end', ()=>{
                cb(status, 4, data, userData)
            })
        })
