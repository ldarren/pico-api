...
run=function(route, idx, ctx, cb){
    ...
    if (!funcs.length){
        funcs.push(...ctx.getKeys(ctx.type))
        funcs[0]=ctx.get(funcs[0])
    }

    for(var i=1,l=funcs.length,f; i<l; i++){
        f=funcs[i]
...
SigSlot.prototype= {
    ...
    signalAt: function(api, at, type){
        // TODO support timer signal
    },
    signalAtEvery: function(api, at, interval, type){
        // TODO support time interval signal
    },
