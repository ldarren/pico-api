...
function quit(state){
	for (let name in this){
		if (this[name].onexit) this[name].onexit()
	}
	if (null != state) process.exit(state)
}
...
module.exports = {
	load(config, cb){
		...
		loadMods(config.mods ? Object.keys(config.mods) : void 0, {config, sigslot:app.sigslot, quit}, (err, context) => {
			function exit(){
				context.quit(0)
			}
